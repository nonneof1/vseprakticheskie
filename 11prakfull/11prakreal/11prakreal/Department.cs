using System;
using System.Collections.Generic;
using System.Numerics;
using System.Text;

namespace _11prakreal
{
    public abstract class Department
    {
        private int departmentId;
        private string name;
        private Doctor headOfDepartment;
        private List<Doctor> doctors;
        private bool[] beds;

        public int DepartmentId
        {
            get { return departmentId; }
        }

        public string Name
        {
            get { return name; }
            set { name = value; }
        }

        public Doctor HeadOfDepartment
        {
            get { return headOfDepartment; }
            set { headOfDepartment = value; }
        }

        protected Department()
        {
            doctors = new List<Doctor>();
        }

        protected Department(int id) : this()
        {
            departmentId = id;
        }

        protected Department(int id, string name) : this(id)
        {
            this.name = name;
        }

        protected Department(int id, string name, Doctor head, int bedCount) : this(id, name)
        {
            headOfDepartment = head;
            beds = new bool[bedCount];
            for (int i = 0; i < bedCount; i++)
                beds[i] = true;
        }

        protected void NotifyDoctors(string message)
        {
            foreach (var doc in doctors)
            {
                Console.WriteLine($"уведомление для доктора {doc.LastName} {doc.FirstName} из отделения {name}: {message}");
            }
        }

        public void AddDoctor(Doctor doctor)
        {
            doctors.Add(doctor);
            NotifyDoctors($"поступил новый врач: {doctor.LastName} {doctor.FirstName}");
        }

        public bool RemoveDoctor(int doctorId)
        {
            var doc = doctors.FirstOrDefault(d => d.DoctorId == doctorId);
            if (doc != null)
            {
                doctors.Remove(doc);
                NotifyDoctors($"врач {doc.LastName} {doc.FirstName} покинул отделение");
                return true;
            }
            return false;
        }

        public Doctor FindDoctor(int doctorId)
        {
            return doctors.FirstOrDefault(d => d.DoctorId == doctorId);
        }

        public int GetAvailableBeds()
        {
            int count = 0;
            foreach (bool bed in beds)
                if (bed) count++;
            return count;
        }

        public int GetDoctorsCount()
        {
            return doctors.Count;
        }

        public abstract string GetDepartmentReport();
        public abstract void SpecializedFunctionality();

        public abstract void NotifyEmergency(string message);
        public abstract void NotifyNewEquipment(string equipment);
    }
}